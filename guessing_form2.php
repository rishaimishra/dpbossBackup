...
?>
<?php if ($res_c->num_rows > 0) {  while($datac=$res_c->fetch_assoc()){?>
			<p><?php  echo $datac['quote']; ?></p>
			<?php } }else{
 ...
 ?>
		...
		<?php if(isset($_SESSION['id'])){  
			...
			<a id="btn-modal-b" class="btn btn-1" onclick="showPost('postid_<?php echo $i; ?>')" data-id="<?php echo $row['post_id']; ?>" data-toggle="modal" data-target="#modal2">Repost</a>
 ...
 <?php }else{ ?>
	...
	<a href="reglogin.php" class="btn btn-1" >Login to Repost</a>
	<?php } ?>
			<a class="btn btn-2" id="btn-modal" onclick="showQuote('postid_<?php echo $i; ?>')" data-id="<?php echo $row['post_id']; ?>" data-toggle="modal" data-target="#modal">(Comment)</a>
		</div>
		<div class="btn-group">
